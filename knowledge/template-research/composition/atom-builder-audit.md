...
| `copy_period_sheet` | planned | source period sheet/block, next period label, clear policy | copy structure, update period labels, clear inputs, preserve formulas | new period sheet/block | new period exists, labels changed, formulas preserved |
| `roll_forward_period` | planned | closing fields, opening fields, carry-forward mapping | bind previous closing to next opening, preserve variance formulas | linked period summary | closing equals next opening, variance remains consistent |
| `normalize_headers` | planned | raw headers, synonym map, required canonical fields | detect header row, map aliases, flag missing or duplicates | canonical header map | required fields resolved, duplicate ambiguity reported |
...
| `add_data_validation` | planned | target ranges, rule type, allowed values or bounds | build dropdown/bounds/date/reference rules | validation rules on input ranges | invalid values rejected or flagged, allowed values pass |
| `protect_formula_cells` | planned | formula ranges, input ranges, lock policy | separate locked formula cells from editable inputs | protected formula regions | formula cells are locked, input cells remain editable |
