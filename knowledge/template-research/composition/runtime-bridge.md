...
`reconcile_tables`, `generate_printable_form`, and `write_values`. The supported `add_data_validation` scope is explicit
...
keys, and tolerance policies remain outside supported scope. The supported
`generate_printable_form` scope writes a fixed printable sheet from explicit
field and table bindings and records a print area; inferred layout, styling
quality, images, merged cells, and PDF pagination remain outside supported
scope.
