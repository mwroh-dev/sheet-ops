...
func TestDraftOrganismExecutionRequestBuildsServiceTicketQueueStepsFromWorkbookFacts(t *testing.T) {
	tempDir := t.TempDir()
	...
	req, ok := DraftOrganismExecutionRequest(OrganismDraftInput{
		ScenarioID:        "service-ticket-organism-draft",
		RequestText:       requestText,
		InputFile:         inputFile,
		OutputFile:        outputFile,
		WorkbookFacts:     facts,
	...
	if req.Steps[0].AtomID != "append_structured_rows" || req.Steps[0].SourceSheet != "Tickets" || len(req.Steps[0].Values) != 4 {
		t.Fatalf("append step=%+v", req.Steps[0])
	}
	if req.Steps[1].AtomID != "extend_table_formulas" || req.Steps[1].FormulaSourceRow != 2 || req.Steps[1].TargetRows[0] != 3 || req.Steps[1].FormulaColumns[0] != "D" {
		t.Fatalf("formula step=%+v", req.Steps[1])
	}
	if req.Steps[2].AtomID != "add_data_validation" || req.Steps[2].ValidationRule == nil || req.Steps[2].ValidationRule.Ranges[0] != "B2:B20" {
		t.Fatalf("validation step=%+v", req.Steps[2])
	}
	if req.Steps[3].AtomID != "highlight_threshold" || req.Steps[3].TargetColumn != "days_open" || req.Steps[3].Threshold == nil || *req.Steps[3].Threshold != 5 {
		t.Fatalf("threshold step=%+v", req.Steps[3])
	}
	if req.Steps[4].AtomID != "group_summarize" || req.Steps[4].TargetSheet != "TicketSummary" || req.Steps[4].Metrics[0].Column != "ticket_count" {
		t.Fatalf("summary step=%+v", req.Steps[4])
	}
	if req.Steps[5].AtomID != "protect_formula_cells" || req.Steps[5].ProtectionRule == nil || req.Steps[5].ProtectionRule.FormulaRanges[0] != "D2:D3" {
...
func TestDraftOrganismExecutionRequestBuildsSalesPipelineStepsFromWorkbookFacts(t *testing.T) {
	tempDir := t.TempDir()
	...
	req, ok := DraftOrganismExecutionRequest(OrganismDraftInput{
		ScenarioID:        "sales-pipeline-organism-draft",
		RequestText:       requestText,
		InputFile:         inputFile,
		OutputFile:        outputFile,
		WorkbookFacts:     facts,
	...
	if req.Steps[0].AtomID != "append_structured_rows" || req.Steps[0].SourceSheet != "Pipeline" || len(req.Steps[0].Values) != 5 {
		t.Fatalf("append step=%+v", req.Steps[0])
	}
	if req.Steps[1].AtomID != "extend_table_formulas" || req.Steps[1].FormulaSourceRow != 2 || req.Steps[1].TargetRows[0] != 3 || req.Steps[1].FormulaColumns[0] != "E" {
		t.Fatalf("formula step=%+v", req.Steps[1])
	...
	if req.Steps[3].AtomID != "highlight_threshold" || req.Steps[3].TargetColumn != "deal_value" || req.Steps[3].Threshold == nil || *req.Steps[3].Threshold != 10000 {
		t.Fatalf("threshold step=%+v", req.Steps[3])
	}
	if req.Steps[4].AtomID != "group_summarize" || req.Steps[4].TargetSheet != "PipelineSummary" || req.Steps[4].Metrics[0].Column != "deal_count" {
		t.Fatalf("summary step=%+v", req.Steps[4])
	}
...
func TestDraftOrganismExecutionRequestBuildsComplianceActionStepsFromWorkbookFacts(t *testing.T) {
	tempDir := t.TempDir()
	...
	req, ok := DraftOrganismExecutionRequest(OrganismDraftInput{
		ScenarioID:        "compliance-action-organism-draft",
		RequestText:       requestText,
		InputFile:         inputFile,
		OutputFile:        outputFile,
		WorkbookFacts:     facts,
	...
	if req.Steps[0].AtomID != "append_structured_rows" || req.Steps[0].SourceSheet != "Compliance" || len(req.Steps[0].Values) != 5 {
		t.Fatalf("append step=%+v", req.Steps[0])
	}
	if req.Steps[1].AtomID != "extend_table_formulas" || req.Steps[1].FormulaSourceRow != 2 || req.Steps[1].TargetRows[0] != 3 || req.Steps[1].FormulaColumns[0] != "E" {
		t.Fatalf("formula step=%+v", req.Steps[1])
	...
	if req.Steps[3].AtomID != "highlight_threshold" || req.Steps[3].TargetColumn != "days_until_due" || req.Steps[3].Operator != "<" || req.Steps[3].Threshold == nil || *req.Steps[3].Threshold != 0 {
		t.Fatalf("threshold step=%+v", req.Steps[3])
	}
	if req.Steps[4].AtomID != "group_summarize" || req.Steps[4].TargetSheet != "ComplianceSummary" || req.Steps[4].Metrics[0].Column != "action_count" {
		t.Fatalf("summary step=%+v", req.Steps[4])
	}
	if req.Steps[5].AtomID != "protect_formula_cells" || req.Steps[5].ProtectionRule == nil || req.Steps[5].ProtectionRule.FormulaRanges[0] != "E2:E3" {
		t.Fatalf("protection step=%+v", req.Steps[5])
	}
	if req.Steps[6].AtomID != "generate_printable_form" || req.Steps[6].TargetSheet != "ComplianceRegister" || req.Steps[6].TableBinding == nil {
