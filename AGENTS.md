...
This repository is a sanitized mirror of the canonical Sheet Ops package. It is
the public install source for `sheet-ops`, not a separate installer-only tree.
...
- `contracts/`, `runtime/`, and `policies/` stay authoritative in release.
- `examples/` provides public request and operation examples referenced by release metadata.
- `internal/testfixtures/` provides release-owned shared verification helpers.
- `knowledge/` carries namespace skeleton and curated sanitized seeds only.
...
Do not treat `.sheet-ops-state/`, `.codex/`, `artifacts/`, `docs/`, `release/`,
or `tests/` as release surfaces. Raw knowledge episode logs such as
`knowledge/**/records.jsonl` do not belong in the release mirror.
...
from a different checkout when this repository is the path the user provided.
